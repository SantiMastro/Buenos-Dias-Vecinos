...
namespace BuenosDias.Tests
{
    ...
    public sealed class SkillcheckDifficultyTests
    {
        ...
        {
            SkillcheckDifficulty difficulty = Build(minimumWidth: 0.12f);
            ReligionDefinition hard = ConfigFactory.Religion(zoneWidth: 0.75f);

            SkillcheckSetup worst = difficulty.Resolve(0f, MaxConverts, 20, hard);

            Assert.AreEqual(0.12f, worst.ZoneWidth, 1e-5f);
        }
